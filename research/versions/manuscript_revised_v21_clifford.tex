...
\begin{table}[t]
\caption{Annealed four-replica point ($n=4$, $d=2$), from exact momentum-zero, colour-resolved spectra of $\Ccomp^{(4)}=M_1M_2$ with full $S_4$ colour resolution at $L=4,6,8,10$.  Rows as in Table~\ref{tab:n3annealed}: consecutive-size crossings of $X_L=L\log(\lambda_1/\lambda_\sigma)$; the amplitude ratio $\log(\lambda_1/\lambda_\sigma)/\log(\lambda_1/\lambda_\varepsilon)$ at the crossing; the raw gap $\log(\lambda_1/\lambda_\sigma)$ (larger size).  The $q=4$ Potts class shares the Ising values $x_\sigma/x_\varepsilon=1/8$ and $1/\nu=3/2$, with multiplicative logarithmic corrections at the marginal point~\cite{Cardy1986}.}
...
size.  In the sense of the sequential Monte Carlo contract this is
...
\bibitem{SM} See Supplemental Material at [URL will be inserted by publisher] for the exceptional-point tables of the three-replica compressed operator ($d=2,3$), the per-block certificate data at $L=12$, the endpoint and $p_\times$ ranks, the complete list of certificate files with SHA-256 hashes and reproduction commands, the proofs of Theorem~\ref{thm:rem} and Proposition~\ref{prop:SMC} (Secs.~S5 and S6), and the Clifford record-count runs of Sec.~\ref{sec:clifforddisorder}: calibration against the deposited tilt chain, the exact $\beta=1$ anchors, the trajectory ladder with its estimator definitions, and reproduction commands (Sec.~S7).
