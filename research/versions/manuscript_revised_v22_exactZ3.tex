% v21 (Clifford record-count closure): integrates the parallel v16_scgf
% line (record SCGF theorem closures) into the canonical chain, with the
% framing corrected to this paper's terminology: the local line's "quenched
% SCGF" Xi_L(beta) = ln E_Born[2^{-beta X_R}]/(2Lt) IS the disorder-direction
% SCGF (the annealed record moment family at fractional order q=1+beta); for
% fixed Clifford omega the record surprisal is omega-measurable (rem:clifford),
% so ALL nontrivial record statistics live in the disorder direction.  New
% Sec. sec:clifforddisorder (prop:cliffscgf: exact beta=1 anchors + Houtappel
% limit -0.08101 nats/site + self-averaging Var/(2Lt)=0.105; prop:gap: the
% L-independent annealed-quenched gap g=0.0226(2) nats/site = the r=1
% interpolation remainder of prop:replica-int, sub-Gaussian 13% correction;
% prop:ess: the collision-tilt ESS law ESS/B=exp[-2Lt(Lambda(2)-2Lambda(1))];
% rem:trichotomy: Gottesman-Knill / 2-design / Haar average-case).  The
% proofs of Theorem thm:rem and Proposition prop:SMC are delegated to the
% Supplemental Material v4 (Secs. S5-S6); the verification chain (deposit
% calibration, t=L/2 convention fix, run inventory) is Supplement Sec. S7.
% Abstract/keywords/intro/Discussion aligned; +3 bibitems
% (Gottesman98, Aaronson-Arkhipov13, Bouland19).
...
\begin{table}[t]
\caption{Annealed four-replica point ($n=4$, $d=2$), from exact momentum-zero, colour-resolved spectra of $\Ccomp^{(4)}=M_1M_2$ with full $S_4$ colour resolution at $L=4,6,8,10$.  Rows as in Table~\ref{tab:n3annealed}: consecutive-size crossings of $X_L=L\log(\lambda_1/\lambda_\sigma)$; the amplitude ratio $\log(\lambda_1/\lambda_\sigma)/\log(\lambda_1/\lambda_\varepsilon)$ at the crossing; the raw gap $\log(\lambda_1/\lambda_\sigma)$ (larger size).  The $q=4$ Potts class has $x_\sigma/x_\varepsilon=(1/8)/(1/2)=1/4$ ($x_\varepsilon=d-1/\nu=1/2$ at $1/\nu=3/2$; the Ising value $1/8$ requires $x_\varepsilon=1$), and $1/\nu=3/2$, with multiplicative logarithmic corrections at the marginal point~\cite{Cardy1986}.}
\label{tab:n4annealed}
\begin{ruledtabular}
\begin{tabular}{lcccc}
 & $(4,6)$ & $(6,8)$ & $(8,10)$ & limit \\
\colrule
$p^*$ & $0.35820$ & $0.37899$ & $0.3823$ & $\approx0.383$ \\
...
assignments rest on the ratio, slope and gap law, not on a proof; the
$n=5$ first-order test is now two-size, its every measured quantity in
the first-order direction.

\begin{remark}[Why $q=n$, and why the annealed points recede from the quenched one]
\label{rem:whyqn}
...
\begin{proof}
The proof---the extremal estimates of item~(ii), the frozen and annealed upper bounds with the Tsirelson--Ibragimov--Sudakov concentration, the truncation lower bound with its countable-intersection uniformity, and the Legendre algebra of the affine branches in item~(v)---is given in the Supplemental Material~\cite{SM}, Sec.~S5.
\end{proof}
...
\begin{remark}[Corners versus affine branches---the physical dichotomy]
\label{rem:dichotomy}
...
\begin{proof}
The Del~Moral induction---the per-event identity
$\E[c_t\widehat\eta_t^N(f)\mid\mathcal F_{t-1}]
=\eta_{t-1}^N(\mathcal Q_{k,\omega}f)$ from the independent predictor
draws, combined with conditionally unbiased resampling and iterated from
$\widehat Z_0=1$---together with the resulting unbiasedness
$\E\widehat Z_T=Z_T$ for $f=1$, the strictness of Jensen's inequality,
and the Radon--Nikodym change-of-measure identity~\eqref{eq:RN}, is given
in the Supplemental Material~\cite{SM}, Sec.~S6.
\end{proof}
...
\subsection{The Clifford record count: trajectory-trivial, disorder-nontrivial}
\label{sec:clifforddisorder}

Remark~\ref{rem:clifford} closes the trajectory direction: for a fixed
Clifford realization $\omega$ (stabilizer input, Pauli measurements,
schedule in $\omega$) the unsigned stabilizer group evolves identically on
every branch, so every record of positive probability carries the same
weight $P_\omega(R)=2^{-N_{\rm rand}(\omega)}$, the surprisal is
$\omega$-measurable, $\psi_{L,\omega}(k)=k\,N_{\rm rand}(\omega)\log2/T$ is
exactly affine, and Theorem~\ref{thm:nofreeze} applies in its degenerate
zero-variance case.  The disorder direction is not trivial:
$N_{\rm rand}(\omega)$ fluctuates over the circuit ensemble with extensive
variance, and its integer moments are exactly the annealed replica
partition functions of Secs.~\ref{sec:compression}--\ref{sec:general-n},
\begin{equation}
 \E_\omega\!\left[2^{(1-n)N_{\rm rand}(\omega)}\right]
 =\E_\omega Z_{n,\omega}(T)=\bar Z_n(t),\qquad n=1,2,\ldots,
 \label{eq:dyadicmoments}
\end{equation}
because $Z_{n,\omega}(T)=\sum_RP_\omega(R)^n=2^{(1-n)N_{\rm rand}}$: the free
cross-check of Remark~\ref{rem:clifford} is the $n=2$ member, and the
$n=3$ member is the object of Sec.~\ref{sec:general-n}.  This subsection
measures the whole family.

\begin{definition}[Disorder SCGF of the record count]
\label{def:cliffscgf}
For the Clifford ensemble of Appendix~\ref{app:numerics} (periodic chain,
$d=2$, $t=\tau L$ periods, $2Lt$ measurement sites) define
\begin{equation}
 \Lambda_L(\beta;\tau,p):=\frac1{2Lt}\,
 \ln\E_\omega\!\left[2^{-\beta N_{\rm rand}(\omega)}\right],\qquad\beta\ge0.
 \label{eq:lambdadef}
\end{equation}
\end{definition}

$\Lambda_L$ is convex and decreasing with $\Lambda_L(0)=0$; in the language
of Definition~\ref{def:records} it is the annealed record moment at
fractional order $q=1+\beta\in[1,2]$ (the disorder average inside the
logarithm, at cloning field $k=-\beta$), \emph{not} a quenched object; at
integer order $\beta=n-1$ it is exactly $(2Lt)^{-1}\ln\bar Z_n(t)$ by
Eq.~\eqref{eq:dyadicmoments}.  Being the scaled cumulant generating
function of the per-site record-entropy density $x=N_{\rm rand}/(2Lt)$ under
the disorder measure (tilt $\theta=-\beta\log2$), it governs, under the
G\"artner--Ellis conditions, the large deviations of $x$: the
trajectory-trivial ensemble has a nontrivial disorder LDP.  The
measurements below combine an exact evolution of the two-replica algebra
on the $3^L$ per-site basis $\{e_0,e_+,e_-\}^L$---calibrated against the
...
simulator that tracks $N_{\rm rand}$ exactly under the Born measure; run
...
\begin{proposition}[disorder SCGF of the Clifford record count]
\label{prop:cliffscgf}
Let $d=2$, $t=\tau L$ with $\tau$ fixed, $p\in\{0.16,0.22\}$.
\begin{enumerate}
\item[(i)] \emph{(exact $\beta=1$ endpoint)}
 $\Lambda_L(1)=(2Lt)^{-1}\ln\bar Z_2(t)$ is computed exactly by the
 transfer matrix of Sec.~\ref{sec:compression} for every $(L,p,t)$:
 $\bar Z_2(t)=A(L,\tau,p)\,\lambda_1(L)^t$ with $A$ subexponential in $t$
 ($A=2.4512$, $4.235$, $7.229$ at $L=8,12,16$ for $p=0.16$, constant to
 six digits for $t\gtrsim3L$; $A=3.279$, $6.833$ at $L=8,12$ for
 $p=0.22$), hence $\Lambda_L(1)\to\log\lambda_1(L)/(2L)$, and by
 Proposition~\ref{prop:houtappel}
 $\lim_L\lim_t\Lambda_L(1)=\tfrac12(\log\mathcal W_0+f_{\rm H})
 =-0.08101$ nats per measurement site at $p=0.16$ (the ladder
 $\log\lambda_1/(2L)=-0.07974,-0.08062,-0.08087,-0.08099,-0.08101$ for
 $L=8,12,16,24,32$; at $p=0.22$: $-0.10870\to-0.11082$).
\item[(ii)] \emph{(quenched density)}
 $\Lambda_L'(0)=-\log2\,\bar x_L$ exactly, with the record-entropy
 density $\bar x_L=\E_\omega N_{\rm rand}/(2Lt)$ converging along the
 trajectory ladder $\bar x_L=0.1448(1),\,0.1470(1),\,0.1480(1),\,
 0.1489(1)$ bits per site at $L=8,12,16,24$ ($p=0.16$; $0.1928\to0.1971$
 at $p=0.22$).
\item[(iii)] \emph{(self-averaging---the measurable criterion)}
 $\operatorname{Var}_\omega(N_{\rm rand})/(2Lt)=0.104$--$0.106$ at
 $p=0.16$ and $0.124$--$0.125$ at $p=0.22$ for $L=8$--$24$:
 $L$-independent, so $\operatorname{Var}(\bar x)=O(1/(Lt))\to0$ and the
 quenched record-entropy density self-averages with an $O(1)$ per-site
 rate---the disorder-LDP backbone, and a direct verification of the
 self-averaging criterion of Proposition~\ref{prop:replica-int}.  The
 two-term cumulant expansion of $\Lambda_L$ reproduces the measured
 $\Lambda_L(0.25)$ to better than $1\%$ at every $L=8$--$24$.
\end{enumerate}
\end{proposition}

\begin{proposition}[the annealed--quenched record gap]
\label{prop:gap}
\begin{enumerate}
\item[(i)] \emph{(finite $L$: exact derivative)} For every finite $L$ the
 replica derivative is exact,
 $\E_\omega[\ln P_\omega(R)]/(2Lt)=\partial_\beta\Lambda_L(0)$: the
 quenched record free-energy density \emph{is} the replica derivative, no
 interchange needed (Proposition~\ref{prop:replica-int} at $m\downarrow0$).
\item[(ii)] \emph{(the interpolation remainder)}
 $g_L:=\Lambda_L(1)+\log2\,\bar x_L
 =(2Lt)^{-1}\bigl(\ln\E_\omega P-\E_\omega\ln P\bigr)$ is exactly the
 $r=1$ remainder of the interpolation identity~\eqref{eq:repint} applied
 to $X=2^{-N_{\rm rand}}$, i.e.\ $g_L=(2Lt)^{-1}\int_0^1(1-s)
 \operatorname{Var}_s(\log X)\,\mathrm ds$.  Measured with the exact
 finite-$t$ $\beta=1$ anchors: $g_L=0.0224$, $0.0226$, $0.0227$ nats per
 site at $L=8,12,16$ ($\approx0.0228$--$0.0229$ at $L=24$ with the
 amplitude-corrected $\lambda_1$ anchor), and $\approx0.027$ at $p=0.22$:
 $L$-independent within the trajectory errors ($\le5\times10^{-4}$).  The
 annealed--quenched separation of the record free energy persists per
 site in the thermodynamic limit---the record analogue of the separation
 $p_c^{(2)}=0.2338$ vs $p_c=0.1597(8)$ of Sec.~\ref{sec:ising}.
\item[(iii)] \emph{(sub-Gaussian correction)} The leading (Gaussian)
 cumulant estimate $(\log2)^2\sigma^2/2=0.0252$ overestimates $g$ by
 $\approx13\%$: the tilted-variance profile decays in $s$.  Equivalently
 the measured $\beta=1$ tilted density $\tilde m(1)=0.0974$ at $L=8$
 shifts by only $65\%$ of the Gaussian prediction
 $\bar x-\log2\,\sigma^2=0.0717$: direct evidence on the profile whose
 $(L,T)$-uniformity the Discussion flags as the open criterion.
\end{enumerate}
\end{proposition}

\begin{proposition}[collision-tilt ESS law]
\label{prop:ess}
The collision tilt $w\propto P_\omega(R)=2^{-N_{\rm rand}}$ reweights the
disorder ensemble, and its effective sample size obeys exactly
\begin{equation}
 \mathrm{ESS}/B=\exp\!\bigl[-2Lt\,(\Lambda_L(2)-2\Lambda_L(1))\bigr].
 \label{eq:esslaw}
\end{equation}
...
suggested.  In the sense of the sequential Monte Carlo contract this is
estimator difficulty, not a physical transition (the clone-collapse
clause); no nonanalyticity of any $\tau$ is involved, and
Theorem~\ref{thm:nofreeze} classifies this family as the degenerate
zero-variance case.  What Eq.~\eqref{eq:esslaw} does measure is extremal
domination of the annealed endpoint by an atypical minority of circuits
(the tilted density sits at $x\approx0.097$ against the typical $0.145$
at $L=8$)---the mechanism whose thermodynamic limit is the freezing of
Theorem~\ref{thm:rem}, appearing here only at the level of a finite-size
estimator.
\end{proposition}

\begin{proof}
Equation~\eqref{eq:esslaw}: with $w_b=2^{-N_b}$, $b=1,\dots,B$ i.i.d.\
disorder draws, $\mathrm{ESS}=(\sum_bw_b)^2/\sum_bw_b^2$, $\E w=2^{2Lt\Lambda_L(1)}$ and $\E w^2=2^{2Lt\Lambda_L(2)}$; the law of large numbers
gives $\mathrm{ESS}/B\to(\E w)^2/\E w^2$ almost surely, which is
Eq.~\eqref{eq:esslaw}; the tabulated values are the within-sample ratios,
and Eq.~\eqref{eq:esslaw} with the trajectory moment estimates reproduces
them at every $L\le16$.  Propositions~\ref{prop:cliffscgf}
and~\ref{prop:gap} quote the deposited runs (Supplemental
Material~\cite{SM}, Sec.~S7): the anchors are exact eigenvalue
evaluations, the trajectory values carry bootstrap standard errors ($200$
resamples), and every calibration check listed there passes.
\end{proof}
...
$\mathrm{ESS}_{\rm true}/B$\\
\hline
8 & 0.16 & -0.077984 & -0.121924 & 0.0340 & 0.0159 & 2.7e-08 \\
8 & 0.22 & -0.106382 & -0.169613 & 0.0432 & 0.0150 & 2.5e-10 \\
12 & 0.16 & -0.079362 & -0.123857 & 0.0349 & 0.0080 & 3.6e-18 \\
12 & 0.22 & -0.108278 & -0.172746 & 0.0438 & 0.0083 & 1.2e-22 \\
16 & 0.16 & -0.079901 & -0.124526 & 0.0353 & 0.0049 & 4.2e-32 \\
16 & 0.22 & -0.109095 & -0.173901 & 0.0443 & 0.0050 & 4.1e-40 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
$0.0227$ exactly.  In the thermodynamic direction, the exact ladder $\ln\lambda_1^{(3)}/(2L) = -0.12478, -0.12586, -0.12605$ nats per site at $L=8,12,16$ ($p=0.16$; $-0.17360, -0.17561, -0.17610$ at $p=0.22$), and the
...
The three levels of the Born-average complexity of the record problem:
(i)~\emph{Clifford realization}: $P(R)$ is dyadic and both the Born
sampling and every tilted average $\E_{\rm Born}[P^\beta f]$ are
poly-time by the Gottesman--Knill theorem~\cite{Gottesman1998}; the
$475\,600$-trajectory quenched locator of Appendix~\ref{app:numerics}
and the exact transfer matrix both live in the efficiently simulable
class, and the dyadic identity~\eqref{eq:dyadicmoments} is verified
operationally (the tableau estimate of $\E[2^{-N_{\rm rand}}]$ matches
the transfer-matrix $\bar Z_2$ to $0.3$--$0.7\%$ with $4\times10^4$
trajectories at $L=8,12$; Supplemental Material~\cite{SM}, Sec.~S7).
(ii)~\emph{any exact $2$-design realization}: the $\beta=1$ endpoint,
the annealed collision $\bar Z_2$, coincides with the Haar value
(Remark~\ref{rem:clifford}), so the exact solvability of the annealed
problem is not a Clifford artifact.
(iii)~\emph{Haar realization}: the per-record probabilities are Born
probabilities of random quantum circuits; approximating them is the stock
average-case-hard task of the sampling
literature~\cite{AaronsonArkhipov2013,Bouland2019} (worst-case \#P-hard;
average-case hardness conditional on standard conjectures), so no
classical poly-time method is known for the quenched Haar record problem.
The Clifford testbed is the maximal exactly-solvable window of the general
monitored problem, and $\Lambda_L$ is well defined precisely because of
level (i).  This complements the worst-case PostBQP completeness of
Sec.~\ref{sec:born}: neither an average-case Haar hardness result nor
hardness of the scalar Born-averaged entropy follows (Discussion).
\end{remark}

One provenance caveat, recorded honestly: the deposited $L=12$ quenched
entropy $2.394(5)$ is not reproduced at either time convention while
both deposited annealed values are (Supplemental Material~\cite{SM},
Sec.~S7); nothing quoted above depends on it.
...
The main unresolved questions are the following.  For $n\ge3$ the operators of Proposition~\ref{prop:general-n} have a real positive semisimple nonzero spectrum, a strict contraction bound, an $S_n\times S_n$ symmetry and an exact compression (Theorem~\ref{thm:general-n}), but no Ising equivalence: A closed-form spectrum, a proof that the leading eigenvalue is simple at small $p$, the generic rank for general $L$ (the symmetry bound~\eqref{eq:rank3} is attained for $L\le10$ but not at $L=12$), and a proof of the existence and regularity of the $L\to\infty$ growth rate are open; scaling-quality spectral data now exist for $n=3$ (Sec.~\ref{sec:n3annealed}) and $n=4$ with full $S_4$ colour resolution (Sec.~\ref{sec:n4n5}): $p_c^{(3)}=0.305(3)$ and $p_c^{(4)}\approx0.383$ at $d=2$, both continuous, with the $q=n$-Potts slope exponents ($1.15$ at $n=3$; $1.00\to1.38\to1.57$ rising through the $q=4$ value $3/2$ with the expected $1/\ln L$ correction at $n=4$) and $\lambda_1$ simple at every tested size and rate—so the remaining $n\ge3$ targets are the growth-rate regularity as a theorem---the two-size confirmation of the first-order prediction is delivered in Sec.~\ref{sec:n4n5} (Table~\ref{tab:n5twosize}), the $q=n$-Potts conjecture is confirmed at the two-size level for $n\le5$, with the $L=8$ rung (the symmetry-reduced block route) in preparation, the $n=4$ chain now extends to $L=10$ with the marginal $q=4$ log-correction analysis, and the $n=3$ moment $\bar Z_3$ itself is now an exact computational object (Prop.~\ref{prop:z3closure}).  For complexity, the Born average $\sum_Rp_RS_n(\rho_{A,R})$ sits exactly outside the BQP-estimable family $M(j,k)$, $j\ge nk$, and neither its hardness nor an average-case Haar statement is known.  For record large deviations, Theorem~\ref{thm:nofreeze} now proves existence, determinism and real-analyticity of the quenched SCGF for every \emph{allowable finite-reachable} process---deterministic, i.i.d.-random, or reducible---and rules out finite-$q$ freezing throughout that class (Corollary~\ref{cor:p1closure} closes the fully monitored Haar-refreshed family unconditionally); the thermodynamic limit beyond finite reachable sets still requires, beyond convexity, control of the finite-volume complex zeros, and whether any \emph{local} circuit ensemble with unbounded reachable set realizes the freezing of Theorem~\ref{thm:rem} with a projectively consistent record process remains open, with the first numerical bounds (Sec.~\ref{sec:smcnumerics}): no linear $\tau$ branch up to $q=3$ at $L\le12$, $T=2L$.  The disorder-replica interchange is reduced by Proposition~\ref{prop:replica-int} to the exact interpolation identity~\eqref{eq:repint} with its monotone replica ladder and measurable self-averaging criterion: the $m\downarrow0$ limit recovers the quenched value quadratically, the $m\to\infty$ limit overshoots it to the extremal value, and what remains open is precisely the uniformity of the tilted variance in $(L,T)$.  (The Clifford record count of Sec.~\ref{sec:clifforddisorder} now supplies one measured profile: an $L$-independent per-site variance of $0.105$, an $L$-independent gap of $0.0226(2)$ nats per site, and a decaying, sub-Gaussian tilted-variance profile---the leading cumulant overshoots the measured gap by $13\%$; the profile \emph{family} is now measured across $(L,T)$---exactly enumerated Haar collision partition functions at $p=1$ over $L=3$--$6$, $T=2L$--$8L$, plus the deposited Clifford disorder ladder at $t=4L$---and the shape metrics are $(L,T)$-stable at the Haar point (the sub-Gaussian ratio $\rho=1.02\pm0.03$ across the whole grid, a flat-to-mildly-rising profile), while the Clifford disorder direction is sub-Gaussian and mildly de-sub-Gaussianizing in $L$ ($\rho=0.872/0.878/0.888$ at $L=8/12/16$, $p=0.16$; $0.890/0.894$ at $p=0.22$)---the first systematic data on the uniformity criterion; Supplemental Material~\cite{SM}, Sec.~S7.)  Finally, the exact integer-$q$ lattice functional is the Haar-averaged $q$-replica channel of Proposition~\ref{prop:general-n}, and the quenched free energy $\E_\omega\log Z_{q,\omega}$ additionally requires the disorder-replica limit $\partial_m\E_\omega Z_{q,\omega}^m|_{m=0}$, an identity that is exact at finite $(L,T)$ and whose interchange with $L,T\to\infty$ and $q\to1$ is reduced by Proposition~\ref{prop:replica-int} to the tilted-variance profile (the self-averaging criterion), the $m\to\infty$ direction being closed negatively (it overshoots to the extremal value); a microscopic derivation of a continuum theory for the quenched problem would have to control all of these limits and reproduce the $S_q\times S_q$ symmetry, the Born normalization of Eq.~\eqref{eq:Mellin}, and exact rather than Gaussian-softened constraints.
...
\bibitem{LePage1974}
E.~Le~Page, \emph{Th\'eor\`emes limites pour les produits de matrices al\'eatoires}, Universit\'e Scientifique et M\'edicale de Grenoble (1974).
...
\bibitem{Gottesman1998} D.~Gottesman, \emph{The Heisenberg representation of quantum computers}, Proc.\ XIXth Int.\ Colloq.\ Group Theoretical Methods in Physics (1998); see also S.~Aaronson and D.~Gottesman, Phys.\ Rev.\ A \textbf{70}, 052328 (2004).
\bibitem{AaronsonArkhipov2013} S.~Aaronson and A.~Arkhipov, Theory Comput.\ \textbf{9}, 143 (2013).
