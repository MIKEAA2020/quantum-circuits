"""Fused numba ring-transfer kernel v2 for the general-n annealed machinery.

v1 of this kernel was correct (1.4e-15 vs the numpy reference) but slow
(48 s vs 60 s at n=5 nb=3): the inner pi-loops had stride-g access on both
work buffers, defeating SIMD.  v2 restructures every loop for stride-1
streaming (pi outer, rest innermost, W's slow third index block-split),
giving the ~2 g^5 MAC contraction at memory-bandwidth-limited speed.
...
def _ring_m2_kernel(v, Wf, g, nb, beta, bufA, bufB, out):
    ...
    for a0 in range(0, g, beta):
        ...
        # stream: (a, tau_0) prange; pi_1 mid (W value hoisted); tail stride-1
        ...
        # ---- steps k = 1..nb-2:
        # cur layout [a, tau_old(g^k), pi, rest']  flat (a*ntau+tau_old)*rest_size + pi*new_rest + rest
        # nxt layout [a, taup(g*ntau),  rest']     flat (a*g*ntau+taup)*new_rest + rest
        # taup = tau_old*g + tau_new
        for k in range(1, nb - 1):
            new_rest = rest_size // g
            blk = new_rest // g          # tail size below the W third digit
        ...
        # out flat = head*g + tau_last ; cur flat = (a*Gt + head)*g + pi
        Gt = Gnb1
        for i in prange(Gt):
            for a in range(b):
                base = (a * Gt + i) * g
                for pi in range(g):
                    ...
                    for tau_last in range(g):
                        out[i * g + tau_last] += c * Wf[tau_last * g * g + bw]
    return out
